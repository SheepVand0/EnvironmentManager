    ...
    internal class EnvironmentManipulator
    {
        ...
        public static async Task<bool> LoadEnvironment()
        {
            ...
            try
            {
                ...
                var l_FirstMap = SongCore.Loader.CustomLevels.First().Value;
                var l_CustomLevelLoader = Resources.FindObjectsOfTypeAll<CustomLevelLoader>().First();
                var l_BeatmapResult = l_CustomLevelLoader.LoadBeatmapLevelData(l_FirstMap);

                var l_PlayerData = Resources.FindObjectsOfTypeAll<PlayerDataModel>().First().playerData;
                EnvironmentsListModel l_Environments = EnvironmentsListModel.CreateFromAddressables();
                AudioClipAsyncLoader l_AudioClipAsyncLoader = AudioClipAsyncLoader.CreateDefault();
                //BeatmapDataLoader l_Loader = BeatmapDataLoader.
                //SettingsManager l_SettingsManager = Resources.FindObjectsOfTypeAll<SettingsManager>().First();
                BeatmapDataLoader l_BeatmapDataLoader = new BeatmapDataLoader();

                var l_Characteristics = l_FirstMap.GetCharacteristics().First();
                BeatmapKey l_BeatmapKey = new BeatmapKey(l_FirstMap.levelID, l_Characteristics, l_FirstMap.GetDifficulties(l_Characteristics).First());
                l_SceneTransitionSetupData.Init(
                    "Standard", l_BeatmapResult, in l_BeatmapKey,
                    l_FirstMap,
                l_PlayerData.overrideEnvironmentSettings,
                l_PlayerData.colorSchemesSettings.GetSelectedColorScheme(),
                l_PlayerData.colorSchemesSettings.colorOverrideType == ColorSchemesSettings.ColorOverrideType.All,
                l_PlayerData.colorSchemesSettings.GetSelectedColorScheme(),
                l_PlayerData.gameplayModifiers, 
                l_PlayerData.playerSpecificSettings,
                l_PlayerData.practiceSettings,
                l_Environments,
                l_AudioClipAsyncLoader,
                SettingsManager.CreateUninitialized(),
                l_BeatmapDataLoader, 
                (BeatmapLevelsEntitlementModel)SongCore.Loader.BeatmapLevelsModelSO.entitlements, string.Empty);
