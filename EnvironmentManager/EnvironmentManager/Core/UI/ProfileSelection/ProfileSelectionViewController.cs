    ...
    internal class ProfileSelectionViewController : ViewController<ProfileSelectionViewController>
    {
        ...
        internal void RefreshProfileList()
        {
            ...
            for (int l_i = 0; l_i < EMConfig.Instance.UserProfiles.Count();l_i++)
            {
                ...
                if (l_i == EMConfig.Instance.SelectedIndex)
                    m_UIProfiles[l_i].OnClick();
        ...
        {
            if (!Directory.Exists("./UserData/EnvironmentManagerImport"))
            {
                Directory.CreateDirectory("./UserData/EnvironmentManagerImport");
                return;
            }
            List<EMConfig.EMProfile> l_Imported = new List<EMConfig.EMProfile>();

            var l_FilesToImport = Directory.GetFiles("./UserData/EnvironmentManagerImport");
            foreach (string l_File in l_FilesToImport)
            {
                if (!l_File.EndsWith(".emp")) continue;

                try
                {
                    EMConfig.EMProfile l_Profile = JsonConvert.DeserializeObject<EMConfig.EMProfile>(File.ReadAllText(l_File));
                    l_Profile.Name += " Imported";
                    l_Imported.Add(l_Profile);
                    File.Delete(l_File);
                } catch
                {
                }
            ...
            EMConfig.Instance.UserProfiles.AddRange(l_Imported);
            EMConfig.Instance.Save();
        }

        protected void ExportProfile()
        {
            var l_SelectedProfile = EMConfig.Instance.UserProfiles[EMConfig.Instance.SelectedIndex];
            if (!Directory.Exists("./UserData/EnvironmentManagerExported"))
            {
                Directory.CreateDirectory("./UserData/EnvironmentManagerExported");
            }

            File.WriteAllText($"./UserData/EnvironmentManagerExported/{l_SelectedProfile.Name}_{CP_SDK.Misc.Time.UnixTimeNow()}.emp", JsonConvert.SerializeObject(l_SelectedProfile, EMConfig.Instance.GetConverters()));
        }
